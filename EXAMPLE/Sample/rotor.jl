...
lP_samples = 1_000, lP_adapts = 500, initθ = randn(3N) .+ 1);

Op = Observe(ClP,C_OCSpH2);
histogram2d(Op[2,:,1,:][:],Op[1,:,1,:][:],bins=(100, 100), show_empty_bins=true,
normalize=:pdf,xlim = (-7, 7),ylim = (0, 7),background_color=:Black)

lP_w,lP,lPststs = runHMC(OCSpH2;
lP_samples = 1000, lP_adapts = 500, initθ = C2Q_init(ClP,OCSpH2))
